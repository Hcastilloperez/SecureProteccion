...
export class AIController {
  async getRecommendations(req: Request, res: Response): Promise<void> {
    try {
      ...
      const recommendations = await prisma.incidentRecommendation.findMany({
        where,
  ...
  async analyzeIncident(req: Request, res: Response): Promise<void> {
    try {
      ...
      const ollamaUrl = urlConfig?.value || config.ollama.baseUrl;
...
Dirección: ${installation.address}
 ...
 4. Medidas preventivas recomendadas`;
  ...
  async analyzeSecurityStudy(req: Request, res: Response): Promise<void> {
    try {
...
6. Plan de acción sugerido`;
  ...
  async getConfigurations(req: Request, res: Response): Promise<void> {
    try {
      ...
      const where: any = installationId ? { installationId: String(installationId) } : {};
  ...
  async createConfiguration(req: Request, res: Response): Promise<void> {
    try {
      ...
      const configuration = await prisma.aIConfiguration.create({
        data: {
          name,
          model: model || 'llama3',
  ...
  async updateConfiguration(req: Request, res: Response): Promise<void> {
    try {
      ...
      const configuration = await prisma.aIConfiguration.update({
        where: { id },
        data: {
          ...(name && { name }),
          ...(model && { model }),
  ...
  async getAvailableModels(req: Request, res: Response): Promise<void> {
    try {
      ...
      const url = baseUrl ? String(baseUrl) : config.ollama.baseUrl;
  ...
  async setDefaultModel(req: Request, res: Response): Promise<void> {
    try {
      ...
      const configModel = baseUrl || config.ollama.model;
      ...
      if (baseUrl && baseUrl !== config.ollama.baseUrl) {
        await prisma.configuration.upsert({
          where: { key: 'OLLAMA_BASE_URL' },
      ...
      res.json({
        ...
        data: { model, baseUrl: baseUrl || config.ollama.baseUrl },
      });
    } catch (error) {
  ...
  async getDefaultConfig(req: Request, res: Response): Promise<void> {
    try {
      const modelConfig = await prisma.configuration.findUnique({
        where: { key: 'OLLAMA_MODEL' },
      });
      ...
      res.json({
        success: true,
        data: {
          model: modelConfig?.value || config.ollama.model,
          baseUrl: urlConfig?.value || config.ollama.baseUrl,
        },
      });
    } catch (error) {
