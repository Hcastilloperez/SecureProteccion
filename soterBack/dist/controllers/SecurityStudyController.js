...
exports.securityStudyController = exports.SecurityStudyController = void 0;
const database_1 = __importDefault(require("../config/database"));
class SecurityStudyController {
    async getAll(req, res) {
        try {
            ...
            const [data, total] = await Promise.all([
                database_1.default.securityStudy.findMany({
                    where,
                    skip,
                    take: limit,
                    orderBy: { createdAt: 'desc' },
                }),
                database_1.default.securityStudy.count({ where }),
            ]);
            res.json({
                success: true,
                data,
    ...
    async getById(req, res) {
        try {
            ...
            const study = await database_1.default.securityStudy.findFirst({
    ...
    async update(req, res) {
        try {
            ...
            const updated = await database_1.default.securityStudy.update({
                where: { id },
                data: {
                    ...(title && { title }),
    ...
    async delete(req, res) {
        try {
            ...
            await database_1.default.securityStudy.delete({ where: { id } });
                ...
                if (jsonMatch) {
                    ...
                    for (const line of lines) {
                        ...
                        if (lowerLine.includes('análisis de amenazas') || lowerLine.includes('threat')) {
                            ...
                            if (currentSection === 'threat')
                                threatAnalysis += line + '\n';
                            else if (currentSection === 'vulnerability')
                                vulnerabilityAnalysis += line + '\n';
                            else if (currentSection === 'recommendations')
                                recommendations += line + '\n';
                            else if (currentSection === 'risk') {
                                const riskMatch = line.match(/(BAJO|MEDIO|ALTO|CRÍTICO|CRITICAL)/i);
                                if (riskMatch)
                                    riskLevel = riskMatch[0].toUpperCase();
                            }
                        }
                    }
                    if (!recommendations)
...
exports.SecurityStudyController = SecurityStudyController;
exports.securityStudyController = new SecurityStudyController();
//# sourceMappingURL=SecurityStudyController.js.map