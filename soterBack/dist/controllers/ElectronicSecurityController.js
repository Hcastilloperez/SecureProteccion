...
exports.electronicSecurityController = exports.ElectronicSecurityController = void 0;
const database_1 = __importDefault(require("../config/database"));
class ElectronicSecurityController {
    ...
    async createSystem(req, res) {
        try {
            ...
            const system = await database_1.default.securitySystem.create({
                data: {
                    name,
                    type,
                    brand,
    ...
    async updateSystem(req, res) {
        try {
            ...
            const system = await database_1.default.securitySystem.update({
                where: { id },
                data: {
                    ...(name && { name }),
                    ...(type && { type }),
    ...
    async deleteSystem(req, res) {
        try {
            const { id } = req.params;
            await database_1.default.securitySystem.delete({ where: { id } });
    ...
    async createEquipment(req, res) {
        try {
            ...
            const { name, type, brand, model, serialNumber, status, location, ipAddress, macAddress, firmwareVersion, expirationDate, notes, specifications, securitySystemId, equipmentTypeId, investmentContractId } = req.body;
            if (!name || !type) {
                res.status(400).json({ success: false, error: 'Campos requeridos faltantes' });
                return;
            }
    ...
    async assignEquipment(req, res) {
        try {
            ...
            const { equipmentId, installationId, securitySystemId, location, latitude, longitude, ipAddress, macAddress, firmwareVersion, notes } = req.body;
    ...
    async deleteEquipment(req, res) {
        try {
            const { id } = req.params;
            await database_1.default.equipment.delete({ where: { id } });
    ...
    async getMaintenanceSchedules(req, res) {
        try {
            ...
            const schedules = await database_1.default.maintenanceSchedule.findMany({
                where,
                include: {
    ...
    async createMaintenanceSchedule(req, res) {
        try {
            ...
            const schedule = await database_1.default.maintenanceSchedule.create({
                data: {
                    title,
                    description,
                    type,
    ...
    async updateMaintenanceSchedule(req, res) {
        try {
            ...
            const schedule = await database_1.default.maintenanceSchedule.update({
                where: { id },
                data: {
                    ...(title && { title }),
                    ...(description !== undefined && { description }),
    ...
    async getInventoryStats(req, res) {
        try {
            const { installationId } = req.query;
            const systemWhere = installationId ? { installationId: String(installationId) } : {};
            const equipmentWhere = installationId ? { securitySystem: systemWhere } : {};
            const [totalSystems, activeSystems, totalEquipments, equipmentStats, upcomingMaintenances, allEquipments, allMaintenances,] = await Promise.all([
                database_1.default.securitySystem.count({ where: systemWhere }),
                database_1.default.securitySystem.count({ where: { ...systemWhere, isActive: true } }),
                database_1.default.equipment.count({ where: equipmentWhere }),
                database_1.default.equipment.groupBy({
                    by: ['status'],
                ...
                database_1.default.equipment.findMany({
                    where: equipmentWhere,
            ...
            const equipmentCosts = allEquipments.reduce((sum, eq) => {
                if (eq.specifications?.cost)
                    return sum + Number(eq.specifications.cost);
                return sum;
            }, 0);
            ...
            res.json({
                ...
                data: {
                    ...
                    stats: {
                        activeEquipments: equipmentStats.find((e) => e.status === 'ACTIVE')?._count || 0,
                        inRepairEquipments: equipmentStats.find((e) => e.status === 'IN_REPAIR')?._count || 0,
                        decommissionedEquipments: equipmentStats.find((e) => e.status === 'DECOMMISSIONED')?._count || 0,
                    },
                },
            });
        }
...
exports.ElectronicSecurityController = ElectronicSecurityController;
exports.electronicSecurityController = new ElectronicSecurityController();
//# sourceMappingURL=ElectronicSecurityController.js.map