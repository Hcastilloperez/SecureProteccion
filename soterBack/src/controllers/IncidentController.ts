...
export class IncidentController {
  ...
  async addTimeline(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user?.userId;
  ...
  async escalate(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user?.userId;
      ...
      if (isRoleGroup) {
        const coordinatorUser = await prisma.user.findFirst({
          where: { role: assignedToId as any, status: 'ACTIVE' },
          orderBy: { createdAt: 'asc' },
        });
        if (coordinatorUser) {
      ...
      const escalatedStatus = await prisma.status.findFirst({
  ...
  async verify(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const verifiedStatus = await prisma.status.findFirst({
        where: { code: 'VERIFIED', type: 'INCIDENT' },
      });

      const invalidStatus = await prisma.status.findFirst({
    ...
    try {
      ...
      const [total, open, inProgress, closed] = await Promise.all([
        ...
        prisma.status
          ...
          .then((status) =>
            status ? prisma.incident.count({ where: { ...where, statusId: status.id } }) : 0
          ),
        prisma.status
      ...
      const byType = await prisma.incident.findMany({
        where,
        select: {
